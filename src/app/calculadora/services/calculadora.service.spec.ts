...
it('deve garantir que 2 ^ 3 = 8', 
  inject([CalculadoraService], (service: CalculadoraService) => {
    let multiplicacao = service.calcular(2, 3, CalculadoraService.EXPONENCIAL);
    expect(multiplicacao).toEqual(8);
  })
);
it('deve garantir que a raiz de 9 é 3', 
  inject([CalculadoraService], (service: CalculadoraService) => {
    let multiplicacao = service.calcular(9, null, CalculadoraService.RAIZ_QUADRADA);
    expect(multiplicacao).toEqual(3);
  })
);
